...
import { rekList } from '@/data/static/rek-list';
...
export default function RekSelectView({ onSelect }: CoinSelectViewTypes) {
    const { closeModal } = useModal();
    let [searchKeyword, setSearchKeyword] = useState('');
    let coinListData = rekList;
    if (searchKeyword.length > 0) {
        coinListData = rekList.filter(function (item) {
        const name = item.account_number;
        return (
    ...
    return (
        ...
        <ul role="listbox" className="min-h-[200px] py-3">
            ...
            coinListData.map((item, index) => (
                ...
                >
                {item.icon}
                <span className="uppercase">{item.account_number}</span>
                </li>
            ))
            ) : (
