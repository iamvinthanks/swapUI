...
import { ListPayment } from '@/data/static/list-payment';

interface PaymentSelectViewTypes {
  onSelect: (selectedCoin: CoinTypes) => void;
}

export default function PaymentSelectView({ onSelect }: PaymentSelectViewTypes) {
  const { closeModal } = useModal();
  let [searchKeyword, setSearchKeyword] = useState('');
  let coinListData = ListPayment;
  if (searchKeyword.length > 0) {
    coinListData = ListPayment.filter(function (item) {
